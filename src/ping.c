...
struct timeval sentTime; //the time that we sent the packet set just before send()
...
#define ECHO_REQ   8
#define ECHO_REPL  0
//#define HOSTADDR   "89.32.148.93" //ip address that i will ping
...
int createRawSocket() {
   ...
   //make sure that i have no error
   if (fd < 0) {
...
int main(int argc, char* argv[]) {
   ...
   //make sure i have enough arguments
   if (argc != 2) {
   ...
   for(i = 0; i < PING_COUNT; i++) {
      //if this isnt the first packet then pasue 
      if (sequence != 0) {
         sleep(1);         
      }
      ...
      //get the time that we are sending the packet
      if (gettimeofday ((struct timeval *)&sentTime, NULL)) {
         printf("Error: problem establishing time before sending the request");
      }

      //send the packet
      if (sendto (socket_fd, buffer, address_length, 0, (struct sockaddr *) &dest_address, 
